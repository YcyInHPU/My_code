...
for i in range(0,100):
     ...
     train_x_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\train_test_split/train_x_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     train_y_reg=pd.DataFrame(y_train)
     train_y_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\train_test_split/train_y_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     test_x_reg=pd.DataFrame(X_test)
     test_x_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\train_test_split/test_x_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     test_y_reg=pd.DataFrame(y_test)
     test_y_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\train_test_split/test_y_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     
     # The training process of ML

     model = RandomForestRegressor(n_estimators = 500, max_depth = None, bootstrap = True, oob_score = True)
     model.fit(X_train,y_train)
     #model_path = 'xgb_test'+'_sat.json'
     #joblib.dump(model, "pima.joblib.dat")
     #loaded_model = joblib.load("pima.joblib.dat")
     joblib.dump(model, r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\model/save_ '+ str(i) + '_model.m')
     #Predict the test set and training set based on trained model
     ans_test = model.predict(X_test)
     result_pre_test=pd.DataFrame(ans_test)
     result_pre_test.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\result_pre/rf_pre_test_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     ans_train = model.predict(X_train)
     result_pre_train=pd.DataFrame(ans_train)
     result_pre_train.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\result_pre/rf_pre_train_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     # ans_last = model.predict(X_last)
     # result_pre_last=pd.DataFrame(ans_last)
     # result_pre_last.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\result_pre/rf_pre_last_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     # # 显示重要特征
...
validation_sta.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\RF\Volume_key_variables_500\validation_sta/rf_key_variables_validation_volume.csv',encoding='gb2312',index=None)
