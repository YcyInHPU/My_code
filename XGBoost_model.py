...
# vars = ['Log10_A', 'Log10_P', 'Cr',  'D', 'Slope_100', 'Slope_200','Slope_500', 'Tmp', 'Pre'] #9因子
# vars = ['Log10_A', 'Log10_P', 'Cr', 'SDI', 'D', 'Slope_100', 'Tmp', 'Pre'] #8因子
# vars = ['Log10_A', 'Slope_100', 'Tmp', 'Pre'] #4因子
...
sheet_name1 = 'last2'
all_data1 = pd.read_excel(path1,sheet_name)
...
for i in range(0,100):
     ...
     train_x_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\train_test_split/train_x_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     train_y_reg=pd.DataFrame(y_train)
     train_y_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\train_test_split/train_y_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     test_x_reg=pd.DataFrame(X_test)
     test_x_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\train_test_split/test_x_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     test_y_reg=pd.DataFrame(y_test)
     test_y_reg.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\train_test_split/test_y_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     
     # The training process of ML机器的训练过程
     #model = xgb.Booster(model_file='model.xgb')
     # model = xgb.XGBRegressor(learning_rate=0.1, n_estimators=140, max_depth=10, min_child_weight=1, seed=0,
     ...
     model.save_model(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\model/save_ '+ str(i) + '_model.xgb')
     ...
     result_pre_test.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\result_pre/xgb_pre_test_ '+ str(i) +'.csv',encoding='gb2312',index=None)
     ans_train = model.predict(X_train)
     result_pre_train=pd.DataFrame(ans_train)
     result_pre_train.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\result_pre/xgb_pre_train_ '+ str(i) +'.csv',encoding='gb2312',index=None)
...
validation_sta.to_csv(r'E:\XM\20220820全国水库库容模型\TEST\XGBoost\Volume_key_variables_500\validation_sta/xgb_key_variables_validation_volume.csv',encoding='gb2312',index=None)

# for i in range(1):
#     plt.subplot(1, 1, i+1)
#     plt.scatter(y_train[:,0], ans_train[:,0], label='Training')
#     plt.scatter(y_test[:,0], ans_test[:,0], label='Validation')
#     plt.plot(range(0, 100), range(0, 100), 'k--')
#     plt.legend()
#     plt.xlim((0, 100))
#     plt.ylim((0, 100))
